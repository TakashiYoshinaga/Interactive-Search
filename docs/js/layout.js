...
//
// z（奥行き）だけは +1 にしている。当てはまりの数字だけ見ると -1 のほうが小さいが、
//  (1) 参照側は z を 0.6 倍に潰しているので、そもそも z の比較は当てにならない
//  (2) 3 軸すべて反転させると行列式が -1 になり「鏡像」になる。z を +1 にすると
//      行列式が +1、つまり純粋な回転になるので、絵の手前・奥の関係が素直に読める
// x, y は変えていないので、上下・左右の向きはそのまま。
const ORIENT = [-1, -1, 1];
